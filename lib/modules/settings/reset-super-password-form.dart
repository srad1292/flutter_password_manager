...
class _ResetSuperPasswordFormState extends State<ResetSuperPasswordForm> {
  ...
  Widget _buildSubmitButton() {
    return ElevatedButton(
      onPressed: formValid && !saving ? () async {
        bool canAdd = await showConfirmationDialog(context: context, body: "Resetting your super password will delete all accounts saved.  Continue?");
        if(!canAdd) { return; }

        resetSuperPassword();
      } : null,
      child: Text(saving ? "Updating..." : "Reset Password"),
      style: ButtonStyle(
  ...
  void resetSuperPassword() async {
    setState(() { saving = true; });
    SuperPassword password = new SuperPassword(
      password: _newPasswordController.text.trim(),
    );

    Function saveFunction = _passwordService.resetSuperPassword;

    try {
      bool didReset = await saveFunction(password);
      setState(() { saving = false; });
      if(didReset) {
        Navigator.of(context).pop();
      } else {
        await showErrorDialog(context: context, body: "Reset super password failed.  Please try again.");
      }
    } catch(err) {
      await showErrorDialog(context: context, body: "Reset super password failed.  Please try again.");
      setState(() { saving = false; });
